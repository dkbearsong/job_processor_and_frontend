...
def execute_transaction_on_remote_db(
    ssh_host: str,
    ssh_port: int,
    ssh_username: str,
    ssh_password: str,
    ...
    """
    Execute multiple queries in a single transaction on a remote PostgreSQL database via SSH tunnel.
    
    Args:
        ... (same as execute_query_on_remote_db)
        queries_and_params (list): List of tuples (sql_query, parameters_dict_or_tuple)
    """
    ...
    if ssh_password:
        ...
        if ssh_key:
            ...
            raise ValueError(f"Could not load SSH key from {ssh_key_path}.")
    ...
    try:
        ...
        logger.error(f"Error connecting to database for transaction: {e}")
        raise

def execute_query_with_env_vars(query: str, params: dict | tuple | None = None):
    """
    Execute a SELECT query using environment variables for configuration.
    
    Args:
        query (str): SQL SELECT query to execute
        params (dict | tuple | None): Query parameters
...
def execute_transaction_with_env_vars(queries_and_params: list):
    """
    Execute a list of queries in a single transaction using environment variables for configuration.
    ...
    return execute_transaction_on_remote_db(
        ssh_host=ssh_host or "",
        ssh_port=ssh_port,
        ssh_username=ssh_username or "",
        ssh_password=ssh_password or "",
